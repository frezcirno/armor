...
public:
   ...
   * @return 是否有效
   */
  virtual bool wait_and_get(cv::Mat &frame, int64 &timeStamp,
...
public:
   ...
   * @return false = 获取失败
   */
  bool wait_and_get(cv::Mat &frame, int64 &timeStamp,
    ...
    func();
    timeStamp =
        int64(1000.0 * 1000.0 * (cv::getTickCount() - m_startTickCount) /
              cv::getTickFrequency());
...
private:
   ...
   * @param pContext
   */
  static void m_connectCallback(CameraHandle hCamera, UINT MSG, UINT uParam,
                                PVOID pContext) {
    // 0: 相机连接断开    1: 相机连接恢复
    if (MSG == 0)
